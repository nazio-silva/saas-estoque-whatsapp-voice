...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redireciona para a página principal (configurações do cliente) após o login
      router.push('/');

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } catch (error: any) {
      console.error('Erro no login:', error);
      setMessage(error.response?.data?.message || 'Credenciais inválidas. Verifique seu e-mail e senha.');
    ...
    <div style={{ fontFamily: 'Arial, sans-serif', maxWidth: '400px', margin: '80px auto', padding: '30px', border: '1px solid #ddd', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <button
          ...
          style={{
            padding: '14px 20px',
            backgroundColor: '#007bff', // Azul para login
            color: 'white',
            border: 'none',
            borderRadius: '6px',
            cursor: 'pointer',
      ...
      <p style={{ textAlign: 'center', marginTop: '25px', color: '#666' }}>
        Não tem uma conta?{' '}
        <Link href="/register" style={{ color: '#28a745', textDecoration: 'none', fontWeight: 'bold' }}>
          Cadastre-se
        </Link>
      </p>
    </div>
